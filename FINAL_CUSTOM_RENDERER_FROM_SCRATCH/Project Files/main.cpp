...
struct Shader : IShader {
    ...
    virtual bool fragment(const vec3 bar, TGAColor &color) {
        ...
        vec3 r = (n*(n*l)*2 - l).normalize(); // reflected light direction, specular mapping
...
int main(int argc, char** argv) {
    ...
    framebuffer.write_tga_file("outputFinal.tga"); // the vertical flip is moved inside the function
    return 0;
}
